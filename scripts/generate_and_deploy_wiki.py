"""
Script to generate the comprehensive, engaging Technical Case Study Blog Post
and deploy it to the GitHub Wiki for the repository.
"""
...
BLOG_POST_CONTENT = r"""# Building a Modern Healthcare Data Platform: A Multi-Cloud Modernization Journey with Azure, Snowflake, Power BI, and Terraform
...
WIKI_HOME_CONTENT = r"""# AMN Healthcare Modern Data Platform Wiki

Welcome to the **AMN Healthcare Modern Multi-Cloud Data Platform & Analytics Engineering Wiki**.

> **⚠️ Learning Project Disclaimer:**  
> This project is a hands-on **educational data engineering case study** inspired by Snowflake's public AMN Healthcare customer story. All datasets are synthetic, simulated, or public reference data (CMS). Published metrics (>100 GB/day, 1,176 tables, 99.9% SLA, 93% cost reduction) are reference benchmarks and architectural design goals, not claims of AMN Healthcare's proprietary systems.

---

## 📚 Wiki Table of Contents

* **[1. Complete Case Study Blog](Case-Study-Blog):** Full in-depth engineering story, background, and results.
* **[2. Solution Architecture](Architecture):** End-to-end data flow, multi-cloud boundaries, and warehouse topologies.
* **[3. Services & Alternatives Breakdown](Services-and-Alternatives):** Detailed review of all 9 cloud services, how they work, alternatives considered, and rationale.
* **[4. Data Pipelines & Engineering Mechanics](Data-Pipelines):** Event-driven routing, metadata ingestion, watermark CDC, and idempotent merges.
* **[5. Power BI DirectQuery Dashboard](Power-BI-Dashboard):** 4-page responsive dashboard suite, star schema model, and DAX catalog.
* **[6. Troubleshooting & Engineering Learnings](Troubleshooting-and-Learnings):** Real-world issues faced, root cause analyses, and solutions.

---

## 🚀 Quick Repository Navigation

* **Word Document Report:** [`AMN_HEALTHCARE_COMPLETE_CASE_STUDY_REPORT.docx`](file:///c:/dev/case_study_01_amn/AMN_HEALTHCARE_COMPLETE_CASE_STUDY_REPORT.docx)
* **ADF Pipelines (JSON):** [`infra/adf/`](file:///c:/dev/case_study_01_amn/infra/adf/)
* **Terraform Infrastructure:** [`infra/terraform/`](file:///c:/dev/case_study_01_amn/infra/terraform/)
* **Snowflake SQL Scripts:** [`sql/snowflake/`](file:///c:/dev/case_study_01_amn/sql/snowflake/)
* **Azure SQL Operational Scripts:** [`sql/azure_sql/`](file:///c:/dev/case_study_01_amn/sql/azure_sql/)
* **Power BI Project (PBIP/TMDL):** [`powerbi/AMN_Healthcare_Dashboard_Presentation/`](file:///c:/dev/case_study_01_amn/powerbi/AMN_Healthcare_Dashboard_Presentation/)
"""

def generate_and_deploy():
    # 1. Write docs/BLOG_POST.md and docs/AMN_HEALTHCARE_CASE_STUDY_BLOG.md
    blog_path = os.path.join(DOCS_DIR, "BLOG_POST.md")
    case_study_blog_path = os.path.join(DOCS_DIR, "AMN_HEALTHCARE_CASE_STUDY_BLOG.md")
    
    print("Writing blog post to docs/...")
    with open(blog_path, "w", encoding="utf-8") as f:
        f.write(BLOG_POST_CONTENT)
    with open(case_study_blog_path, "w", encoding="utf-8") as f:
        f.write(BLOG_POST_CONTENT)

    # 2. Write wiki directory files
    os.makedirs(WIKI_DIR, exist_ok=True)
    wiki_home = os.path.join(WIKI_DIR, "Home.md")
    wiki_blog = os.path.join(WIKI_DIR, "Case-Study-Blog.md")
    
    print("Writing wiki files...")
    with open(wiki_home, "w", encoding="utf-8") as f:
        f.write(WIKI_HOME_CONTENT)
    with open(wiki_blog, "w", encoding="utf-8") as f:
        f.write(BLOG_POST_CONTENT)

    # 3. Attempt to push to GitHub Wiki repository
    try:
        token = subprocess.check_output(["gh", "auth", "token"], text=True).strip()
        wiki_remote_url = f"https://x-access-token:{token}@github.com/sherazi-412002/snowflake_azure_amn_healthcare_case_study.wiki.git"
        
        scratch_wiki = os.path.join(ROOT_DIR, "scratch_wiki_deploy")
        if os.path.exists(scratch_wiki):
            shutil.rmtree(scratch_wiki, ignore_errors=True)
        os.makedirs(scratch_wiki, exist_ok=True)
        
        # Copy all wiki files to scratch
        for item in os.listdir(WIKI_DIR):
            s = os.path.join(WIKI_DIR, item)
            d = os.path.join(scratch_wiki, item)
            if os.path.isfile(s):
                shutil.copyfile(s, d)
        
        cwd = os.getcwd()
        os.chdir(scratch_wiki)
        subprocess.run(["git", "init"], check=True, capture_output=True)
        subprocess.run(["git", "config", "user.name", "sherazi-412002"], check=True, capture_output=True)
        subprocess.run(["git", "config", "user.email", "sherazi@example.com"], check=True, capture_output=True)
        subprocess.run(["git", "add", "."], check=True, capture_output=True)
        subprocess.run(["git", "commit", "-m", "Deploy complete case study blog and wiki pages"], check=True, capture_output=True)
        
        push_res = subprocess.run(["git", "push", "--force", wiki_remote_url, "master"], capture_output=True, text=True)
        os.chdir(cwd)
        shutil.rmtree(scratch_wiki, ignore_errors=True)


        if push_res.returncode == 0:
            print("Successfully deployed directly to GitHub Wiki repository!")
            return True
        else:
            print("GitHub Wiki endpoint returned:", push_res.stderr)
            return False
