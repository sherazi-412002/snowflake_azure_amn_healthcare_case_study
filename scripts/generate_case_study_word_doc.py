"""
Script to generate the comprehensive, exhaustive AMN Healthcare Case Study Report
in Markdown and compile it to a Word Document (.docx) via Pandoc.
"""
...
MARKDOWN_CONTENT = r"""# AMN HEALTHCARE MODERN DATA PLATFORM & ANALYTICS ENGINEERING CASE STUDY
...
def generate_report():
    md_file_path = os.path.join(DOCS_DIR, "AMN_HEALTHCARE_COMPLETE_CASE_STUDY_REPORT.md")
    docx_file_path_docs = os.path.join(DOCS_DIR, "AMN_HEALTHCARE_COMPLETE_CASE_STUDY_REPORT.docx")
    docx_file_path_root = os.path.join(ROOT_DIR, "AMN_HEALTHCARE_COMPLETE_CASE_STUDY_REPORT.docx")
    docx_legacy_path = os.path.join(DOCS_DIR, "AMN_HEALTHCARE_COMPLETE_CASE_STUDY.docx")

    print(f"Writing markdown content to {md_file_path}...")
    with open(md_file_path, "w", encoding="utf-8") as f:
        f.write(MARKDOWN_CONTENT)

    print(f"Compiling Markdown to DOCX via Pandoc...")
    cmd = [
        "pandoc",
        md_file_path,
    ...
    if result.returncode != 0:
        print(f"Error compiling DOCX: {result.stderr}")
        return False
    
    print(f"Copying compiled DOCX to root workspace and updating legacy docx...")
    import shutil
    shutil.copyfile(docx_file_path_docs, docx_file_path_root)
    shutil.copyfile(docx_file_path_docs, docx_legacy_path)

    print("Successfully generated Word Documents:")
    print(f" - {docx_file_path_docs}")
    print(f" - {docx_file_path_root}")
    print(f" - {docx_legacy_path}")
    return True
